...
export async function sendContactEmail(formData: ContactFormData) {
  try {
    ...
    const rateLimitResult = rateLimit(
      getIdentifier(null, formData.email, "contact"),
      "CONTACT_FORM"
    );
    if (!rateLimitResult.success) {
      return { error: `Too many submissions. Please try again in ${Math.ceil((rateLimitResult.retryAfter || 60) / 60)} minutes.` };
    }

    // Validate input with Zod
    const validatedInput = contactSubmissionSchema.safeParse({
          ...
          <body>
            ...
            <div class="content">
              ...
              <div class="field">
                ...
                <div class="message-box">${formData.message.replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\n/g, '<br>')}</div>
              </div>
