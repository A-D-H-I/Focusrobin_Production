...
export default function UniqueDesignsPage() {
  ...
  const [uniqueDesignIds, setUniqueDesignIds] = useState<Set<string>>(new Set());

  useEffect(() => {
    fetchProducts();
    fetchUniqueDesignProducts();
  }, []);
  ...
  const fetchUniqueDesignProducts = async () => {
    try {
      const response = await fetch("/api/admin/products?isUniqueDesign=true");
      if (response.ok) {
        const data = await response.json();
        if (Array.isArray(data)) {
          const ids = new Set(data.map((p: Product) => p.id));
          setUniqueDesignIds(ids);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast({
          title: "Success",
          description: "Unique designs updated successfully",
        });
        setUniqueDesignIds(new Set(selectedProducts));
      } else {
        const errorData = await response.json().catch(() => ({}));
        toast({
          title: "Error",
        ...
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-muted-foreground">
              Select products to display in the "Unique Designs" section on the homepage (formerly Best Sellers)
            </p>
          </div>
        ...
        {products.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {products.map((product) => {
              ...
              const isUniqueDesign = uniqueDesignIds.has(product.id);
              const mainImage =
