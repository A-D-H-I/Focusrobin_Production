...
import { 
  createPrescriptionGlassesLandingImage, 
  updatePrescriptionGlassesLandingImage, 
  deletePrescriptionGlassesLandingImage, 
  setActivePrescriptionGlassesLandingImage 
} from '@/app/actions/prescriptionGlassesLandingImage';
...
interface PrescriptionGlassesLandingImage {
  id: string;
  imageUrl: string;
...
interface PrescriptionGlassesLandingImageManagementProps {
  initialImages: PrescriptionGlassesLandingImage[];
}

export function PrescriptionGlassesLandingImageManagement({ initialImages }: PrescriptionGlassesLandingImageManagementProps) {
  const router = useRouter();
  const { toast } = useToast();
  const [images, setImages] = useState<PrescriptionGlassesLandingImage[]>(initialImages);
  ...
  const handleEdit = (image: PrescriptionGlassesLandingImage) => {
    setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        result = await updatePrescriptionGlassesLandingImage(form);
      } else {
        result = await createPrescriptionGlassesLandingImage(form);
      }
      ...
      if (result.error) {
        ...
        toast({
          title: 'Success',
          description: editingId ? 'Landing image updated successfully' : 'Landing image created successfully',
        });
        resetForm();
        router.refresh();
      }
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this landing image?')) {
      return;
    }
    ...
    try {
      const result = await deletePrescriptionGlassesLandingImage(form);
      if (result.error) {
        toast({
          title: 'Error',
          description: result.error,
  ...
  const handleSetActive = async (id: string) => {
    ...
    try {
      const result = await setActivePrescriptionGlassesLandingImage(form);
      if (result.error) {
        toast({
          title: 'Error',
          description: result.error,
        ...
        toast({
          title: 'Success',
          description: 'Active landing image updated',
        });
        router.refresh();
      }
    } catch (error) {
      ...
      <div className="flex justify-end">
        ...
        >
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingId ? 'Edit Landing Image' : 'Add New Landing Image'}</DialogTitle>
              <DialogDescription>
                Configure the prescription glasses landing page banner image
              </DialogDescription>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="Prescription Glasses Collection"
                  required
                />
              </div>
      ...
      {images.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center">
            <p className="text-muted-foreground mb-4">No landing images found.</p>
            <Button
              onClick={() => {
                resetForm();
                setIsDialogOpen(true);
            ...
            <Card key={image.id} className={image.isActive ? 'ring-2 ring-primary' : ''}>
              <CardHeader>
                <div className="flex items-start justify-between">
                  <CardTitle className="text-lg">Landing Image</CardTitle>
                  {image.isActive && (
