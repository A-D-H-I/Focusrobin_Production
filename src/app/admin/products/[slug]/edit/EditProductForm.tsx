...
import { updateProduct, type VariantData } from '@/app/actions/updateProduct';
...
export function EditProductForm({ product, productId }: EditProductFormProps) {
  ...
  const [variants, setVariants] = useState<VariantData[]>(() => {
    ...
    return product.ProductVariant?.map((variant: any) => {
      const assets = variant.ProductAsset || [];
      ...
      return {
        ...
        asset_glb: glbAsset?.url || '',
        asset_tryon: tryonAsset?.url || '',
        asset_hover: hoverAsset?.url || '',
        asset_gallery: galleryAssets.join(', '),
      };
    }) || [{
      name: '',
      sku: '',
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="space-y-2">
              ...
              <Input id="name" name="name" required defaultValue={product.name} placeholder="e.g., The Horizon" />
            </div>
            <div className="space-y-2">
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          {variants.map((variant, index) => (
            <div key={variant.id || index} className="space-y-4 rounded-lg border p-4">
              <div className="flex items-center justify-between">
